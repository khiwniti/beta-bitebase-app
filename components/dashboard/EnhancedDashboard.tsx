...
export default function EnhancedDashboard({ className }: EnhancedDashboardProps) {
  ...
  const { analytics, loading: analyticsLoading, error: analyticsError, refetch: refetchAnalytics } = useAnalyticsData();
  const { locationData, loading: locationLoading, error: locationError, refetch: refetchLocation } = useLocationIntelligence();
  const { restaurants: recentRestaurants, loading: restaurantsLoading, error: restaurantsError, refetch: refetchRestaurants } = useRestaurantExplorer();
    ...
    try {
      switch (tabName) {
        ...
        case 'analytics':
          // Load analytics data
          break;
        case 'restaurant-explorer':
          // Load restaurant explorer data
          break;
        case 'ai-insights':
          // Already loaded in overview
          break;
        case 'settings':
          // Load user settings
          break;
  ...
  const getInsightIcon = (type: string) => {
    switch (type) {
      ...
      case 'trend': return <Zap className="h-4 w-4 text-purple-600" />;
